...
//calculamos el total de calificaciones de reseñas
    	...
    	rating = Math.ceil(totalReview/JSON.parse(response.reviews).length);
...
export let DinamicReviews = {
	...
	fnc: function(response){
		...
		//Clasificamos la cantidad de estrellas segun la clasificacion
	...
	fnc: function(response){
    	...
    	if(response.offer != ""){
    		...
    		if(today <= offerDate){
	            ...
	            disccount =  `<p class="ps-product__badge">-${value}%</p>`
        ...
        if(response.stock == 0 ){
             ...
             disccount =  `<p class="ps-product__badge out-stock">Out Of Stock</p>`
