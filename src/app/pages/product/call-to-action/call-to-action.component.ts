	  		 	...
	  		 	for(const i in resp){
	  		 		...
	  		 		this.call_to_action.forEach(response =>{
				    	...
				    	if(response.offer != ""){
				            ...
				            this.price.push(`<span class="ps-product__price"><span>$${offer}</span><del>$${response.price}</del></span>`);

				    	}else{

				            this.price.push(`<span class="ps-product__price"><span>$${response.price}</span></span>`);
				        }
